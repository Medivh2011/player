...
SoakJavaCall::SoakJavaCall(_JavaVM *vm, JNIEnv *env, jobject *obj) {
    javaVM = vm;
    jniEnv = env;
    jobj = *obj;
...
//        LOGE("find jclass faild");
    ...
    jmid_video_size_changed = jniEnv->GetMethodID(jlz,"onVideoSizeChanged","(IIF)V");
}

SoakJavaCall::~SoakJavaCall() {
//    LOGE("SoakJavaCall()");
}

void SoakJavaCall::onError(int type, int code, const char *msg) {
    if(type == SOAK_THREAD_CHILD)
    {
        JNIEnv *jniEnv;
...
void SoakJavaCall::onLoad(int type, bool load) {
    if(type == SOAK_THREAD_CHILD)
    {
        JNIEnv *jniEnv;
        if(javaVM->AttachCurrentThread(&jniEnv, 0) != JNI_OK)
...
void SoakJavaCall::onParpared(int type) {
    if(type == SOAK_THREAD_CHILD)
    {
        JNIEnv *jniEnv;
        if(javaVM->AttachCurrentThread(&jniEnv, 0) != JNI_OK)
...
void SoakJavaCall::onInitMediacodec(int type, int mimetype, int width, int height, int csd_0_size, int csd_1_size, uint8_t *csd_0, uint8_t *csd_1) {
...
void SoakJavaCall::onDecMediacodec(int type, int size, uint8_t *packet_data, int pts) {
    if(type == SOAK_THREAD_CHILD)
    {
        JNIEnv *jniEnv;
        if(javaVM->AttachCurrentThread(&jniEnv, 0) != JNI_OK)
...
void
SoakJavaCall::onGlRenderYuv(int type, int width, int height, uint8_t *fy, uint8_t *fu, uint8_t *fv) {
    if(type == SOAK_THREAD_CHILD)
    {
        JNIEnv *jniEnv;
        if(javaVM->AttachCurrentThread(&jniEnv, 0) != JNI_OK)
...
void SoakJavaCall::onVideoInfo(int type, int currt_secd, int total_secd) {
    if(type == SOAK_THREAD_CHILD)
    {
        JNIEnv *jniEnv;
        if(javaVM->AttachCurrentThread(&jniEnv, 0) != JNI_OK)
...
void SoakJavaCall::onComplete(int type) {
    if(type == SOAK_THREAD_CHILD)
    {
        JNIEnv *jniEnv;
        if(javaVM->AttachCurrentThread(&jniEnv, 0) != JNI_OK)
...
bool SoakJavaCall::isOnlySoft(int type) {
    bool soft = false;
    if(type == SOAK_THREAD_CHILD)
    {
        JNIEnv *jniEnv;
...
void SoakJavaCall::onVideoSizeChanged(int type, int width, int height, float dar) {
    ...
    if(type == SOAK_THREAD_CHILD)
    {
        ...
        jniEnv->CallVoidMethod(jobj, jmid_video_size_changed,width,height,dar);
        javaVM->DetachCurrentThread();
    }
    else
    {
         jniEnv->CallBooleanMethod(jobj, jmid_onlysoft);
    }
}
